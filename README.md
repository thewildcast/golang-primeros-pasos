...
Para realizar la suma de dos numeros ya tenes una funcion a tu disposicion que se llama `Sumar` y recibe dos enteros. El problema aca es que `Sumar` tarda una cantidad de tiempo indeterminado (pero no mayor a medio segundo) en retornarte la suma, entonces hacer una suma secuencial de muchos numeros nos puede llegar a tardar mucho tiempo. Para poder hacerla mas eficientes vas a usar goroutines que paralelicen estas operaciones y lograr reducir el tiempo que toma sumar todo. Una forma de encarar esto seria usando semaforos y mantener una variable global de suma que cada goroutine incremente, el problema es que esto no esta acorde a uno de los [proverbios mas importantes de Go](https://go-proverbs.github.io/):
